  ...
  async function getcategory(queryname){
    ...
    let  response = await fetch(`https://cors-anywhere.herokuapp.com/https://www.freetogame.com/api/games?category=${queryname}`);
   ...
   getcategory( navename)
    ...
    async function getdetails(gameId,queryname) {
        let response = await fetch(`https://cors-anywhere.herokuapp.com/https://www.freetogame.com/api/game?id=${gameId}`);
        if(response.ok){
          let data = await  response.json()
          console.log(data)
